...
# TODO (yiakwy) : should be fused with softmax topk score and normalization ?
def permute(tokens, indices, expand_factor: int = 1, is_fp8=False):
    ...
    # NOTE(yiakwy) : used to validate row_map_id generated by permute kernel
    host_row_id_map = [None] * num_tokens * expand_factor
    values = sorted_indices.detach().cpu().numpy()
    for i, val in enumerate(values):
        row = val // expand_factor
        col = val % expand_factor
        
        host_row_id_map[row + col * num_tokens] = i
    
    return permuted_tokens, sorted_indices, np.array(host_row_id_map, dtype=np.int32)
...
def _is_int8(dtype):
    return dtype in (torch.int8, )

def _is_float(dtype):
    return dtype in (torch.double, torch.float32, torch.float16, torch.bfloat16 )

# TODO (yiakwy) : refactor, move out benchmark test
def permute_topK_test(
    ...
    is_int8 = _is_int8(dtype)
    is_float = _is_float(dtype)
    
    # NOTE(yiakwy) : at least 16 bit IO is needed (65000 vocabulary size) for modern LLM application
    is_8_bit_io = False
    is_16_bit_io = False
    
    TORCH_MAJOR_VER, TORCH_MINOR_VER, _ = torch.__version__.split('.')
    
    # intiallize
    # TODO (yiakwy) : check software version
    if is_float and not is_fp8:
        # NOTE(yiakwy) : torch.rand is implemented for device float type, but not for fp8 data type
        permute_input = torch.rand((num_token, hidden_size), dtype=dtype, device="cuda")
    else:
        warnings.warn(f"[permute_topK_test] torch.rand is not implemented for onchip data type {dtype}, use fp16 instead.")
        permute_input = torch.rand((num_token, hidden_size), dtype=torch.float16, device="cuda")
                
    if is_8_bit_io:
        # NOTE(yiakwy) : since FP8 has already been supported in latest pytorch, just return the data
        # TODO (yiakwy) : check version
        if (TORCH_MAJOR_VER, TORCH_MINOR_VER) >= (2,2):
            if permute_input.dtype != dtype: # not _is_fp8(permute_input.dtype) or not _is_int8(permute_input.dtype)
                assert is_fp8 or is_int8
                permute_input = permute_input.to(dtype)
            else:
                if is_float:
                    warnings.warn(f"[permute_topK_test] casting input of dtype {dtype} to torch.float8_e4m3f")
                    dtype = torch.float8_e4m3fn
                else:
                    warnings.warn(f"[permute_topK_test] casting input of dtype {dtype} to torch.int8")
                    dtype = torch.int8
                    
                permute_input = permute_input.to(dtype)
        else:
            # NOTE(yiakwy) : store float8 as int8 and cast int8 to fp8 onchip
            raise Exception("8 bit io is not supported for torch version {TORCH_MAJOR_VER}.{TORCH_MINOR_VER} < 2.2")
    elif is_16_bit_io:
        if _is_float(permute_input.dtype):
            if permute_input.dtype not in (torch.float16, torch.bfloat16):
                warnings.warn(f"[permute_topK_test] casting input of dtype {dtype} to torch.float16")
                dtype = torch.float16
                permute_input = permute_input.half()
        else:
            if permute_input.dtype not in (torch.int16,):
                warnings.warn(f"[permute_topK_test] casting input of dtype {dtype} to torch.int16")
                dtype = torch.int16
                permute_input = permute_input.to(torch.int16)
    ...
    with torch.no_grad():
        if num_token > 0:
            ...
            indices = torch.empty((num_token, num_topK), dtype=torch.int32, device="cuda")
    ...
    permute_output, sorted_indices, host_row_id_map = permute(permute_input, indices, num_topK, is_fp8)
    nvtx.range_pop()
    ...
    #
    ###################################################################################################################################
    new_permute_input = permute_input.detach()
    new_permute_bwd_input = permute_bwd_input.detach()
    new_unpermute_bwd_input = unpermute_bwd_input.detach()
    new_permute_input.requires_grad_(True)
    
    new_permute_output, row_id_map = permute_topK(new_permute_input, indices)

    torch.testing.assert_close(permute_output.float(), new_permute_output.float(), atol=1e-2, rtol=0)
    torch.testing.assert_close(host_row_id_map, row_id_map.detach().cpu().numpy())
    ...
    print(f"premute fwd passed, compute unpermute fwd/bwd")
    print(f"new_probs=\n{new_probs}")
    ...
    # New method V2 accuracy test
    new_permute_input_clone = permute_input.detach()
    new_permute_bwd_input_clone = permute_bwd_input.detach()
    new_unpermute_bwd_input_clone = unpermute_bwd_input.detach()
    new_permute_input_clone.requires_grad_(True)
    
    # use faster permute method
    new_permute_output_v2, row_id_map_v2 = permute_topK(new_permute_input_clone, indices, use_fast_permute=True)

    assert torch.allclose(permute_output.float(), new_permute_output_v2.float())

    if PRINT:
        print("--------------row_id_map_v2--------------")
        print(row_id_map_v2)
        print("--------------new_permute_output_v2--------------")
        print(new_permute_output_v2)
    ...
    if BENCHMARK:
        ...
        t = perf_test_cuda_kernel(lambda: permute_topK(new_permute_input_clone, indices, use_fast_permute=True))
        print(f"faster  fwd: {t:.3f} ms")
...
def test_permute_topK():
    ...
    # special case `num_token == 0`
    # permute_topK_test(dtype, 0, 8, hidden_size, 5, False, Benchmark)
    
    if device_capability[0] < 9:
        warnings.warn(f"[test_permute_topK] fp8 is supported for cuda capability >= 9.0")
        return
    
    # NOTE(yiakwy) : there are problems for kernels to be executed with FP8
...
def parse_args():
    parser = argparse.ArgumentParser(description="OpTest && benchmark for MoE related ops")
